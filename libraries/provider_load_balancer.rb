...
class GaloshesLoadBalancer < Chef::Provider::GaloshesBase
...
def load_current_resource
  ...
  if !(@current_resource.elb.nil?)
    Chef::Log.debug("current.listeners: #{@current_resource.elb.listeners}")
  end
...
def action_create
  ...
  if cur_elb.nil?
    converge_by("Create ELB #{new_resource.name} from scratch") do
      options = {
